...
instance FromJSON TopLevelElt where
  parseJSON (Object v) = TopLevelElt <$> v .:   "status" <*> v .:   "chainid" <*> v .:   "entryhash"
  parseJSON _          = mzero


instance ToJSON TopLevelElt where
  toJSON     (TopLevelElt {..}) = object ["status" .= topLevelEltStatus, "chainid" .= topLevelEltChainid, "entryhash" .= topLevelEltEntryhash]
  toEncoding (TopLevelElt {..}) = pairs  ("status" .= topLevelEltStatus<>"chainid" .= topLevelEltChainid<>"entryhash" .= topLevelEltEntryhash)
